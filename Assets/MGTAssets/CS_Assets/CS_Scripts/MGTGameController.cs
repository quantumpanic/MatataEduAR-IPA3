...
namespace MatchingGameTemplate
{
    ...
    public class MGTGameController : MonoBehaviour
    {
        ...
        [Tooltip("The maximum number of repeating pairs that will appear")]
        ...
        public void SelectObject(Transform selectedObject)
        {
            ...
            else // Selecting the second object of a pair
            {
                ...
                // If we selected two objects, check if they match. In Image - Text mode we compare the image of the object to the name of the other object. In Text - Text mode we compare the pairs and see if they have the same firstText and secondText values
                    ...
                    pairsType == "ImageImage" && (firstObject.Find("Match").GetComponent<Text>().text == selectedObject.Find("Match").GetComponent<Text>().text) && firstObject.parent != selectedObject.parent
                    )
                {
		...
		IEnumerator LevelUp(float delay)
        {
            ...
            if (currentPair < pairs.Length || currentPair < pairsTextText.Length || currentPair < pairsImageSound.Length || currentPair < pairsImageImage.Length && currentLevel < totalRounds)
            {
            ...
            if (defaultButton && pairsImageImage.Length > 0)
            {
                ...
                for (index = currentPair; index < pairsImageImage.Length; index++)
                {
                    ...
                    if (firstOfPair == true)
                    {
                        ...
                        // for unidentical images that need to be matched
                        newButton.Find("Match").GetComponent<Text>().text = pairsImageImage[index].firstImage.name;

                        // for unidentical images that need to be matched of similar types
                        //newButton.Find("Match2").GetComponent<Text>().text = pairsImageImage[index].secondImage.name;
        ...
        IEnumerator GameOver(float delay)
        {
            ...
            yield return new WaitForSeconds(delay);
            GameObject.Find("Objectives").GetComponent<GameManagerClass>().GameOver();

            // override for our game here
        ...
        IEnumerator Victory(float delay)
        {
            ...
            // override for our game here
            GameObject.Find("Objectives").GetComponent<GameManagerClass>().Victory();
